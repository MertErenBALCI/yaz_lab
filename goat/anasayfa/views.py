...
def get_yayin_detay(request, yayin_adi):
    ...
    yayin = hizboCol.find_one({'yayin_adi': yayin_adi})
    ...
    if request.method == 'POST':
        ...
        if keyword:
            ...
            for article in articles[:10]:
                ...
                inserted_doc = hizboCol.insert_one({
                    'yayin_id': counter,
                    'yayin_adi': title,
                    'yayin_hrefi': href,
                ...
                alinti1_yayinlar = hizboCol.find(
                    {'anahtar_kelime_arama': keyword, 
                ...
                filtered_yayinlar = hizboCol.find(
                    {'anahtar_kelime_arama': keyword, 
                    'alinti_sayisi': {'$ne': None},
                    'doi': {'$ne': None}
                    },
                ...
                result2 = hizboCol.find({'anahtar_kelime_arama': keyword}, {'yayin_adi': 1}).limit(10).sort([('yayimlanma_tarihi', -1)])
                yayin_adi_list_kb = [data['yayin_adi'] for data in result2]

                
                result3 = hizboCol.find({'anahtar_kelime_arama': keyword}, {'yayin_adi': 1}).limit(10).sort([('yayimlanma_tarihi', 1)])
